""" ffb6d dataset contain 'p2r_ds_nei_idx%d' and 'p2r_up_nei_idx%d' which is adapted for network """
...
def crop_square_resize(img, Bbox, crop_size=None, interpolation=None):
    ...
    if (y1 > img.shape[0] and y2 > img.shape[0]) or (y1 < 0 and y2 < 0):
        y1 = 0
        y2 = 0    
    if (x1 > img.shape[1] and x2 > img.shape[1]) or (x1 < 0 and x2 < 0):
        x1 = 0
        x2 = 0 
